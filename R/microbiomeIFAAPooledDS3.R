...
microbiomeIFAAPooledDS3 <- function(SumExp,
                                    microbVar,
                                    testCov,
                                    ctrlCov,
                                    sampleIDname,
  ...
  #### left over in order to check for other imported objects from the client side
  finalIndpRefTax <- unlist(strsplit(finalIndpRefTax, split = ","))
  goodIndpRefTaxNam <- unlist(strsplit(goodIndpRefTaxNam, split = ","))
  ...
  if (verbose) {
    results$analysisResults <- int.Regulariz.IFAA3(
      data = data,
